...
package org.antlr.v5.runtime._unused.pattern;
 ...
 * {@link ParseTreeMatch#getMismatchedNode} set to the first tree node that did not
 * match.</p>
 *
		...
		for (Chunk chunk : chunks) {
			if ( chunk instanceof TagChunk ) {
				...
				if ( Character.isUpperCase(tagChunk.getTag().charAt(0)) ) {
					...
					int ruleImaginaryTokenType = parser.getAtnWithBypassAlts().getRuleToTokenType()[ruleIndex];
